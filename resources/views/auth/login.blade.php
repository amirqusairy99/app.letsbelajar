<x-guest-layout>
    <div class="card shadow-sm border-0 rounded-3" style="border-color: var(--js-border-strong) !important; background: rgba(30, 41, 59, 0.8); backdrop-filter: blur(12px);">
        <div class="card-body p-4">
            <h2 class="h4 mb-1 fw-bold" style="color: var(--js-text-primary);">Welcome back</h2>
            <p class="mb-4" style="color: var(--js-text-muted); font-size: 0.875rem;">Sign in to your LetsBelajar account</p>

            <form method="POST" action="{{ route('login') }}">
                @csrf
                <div class="mb-3">
                    <label for="email" class="form-label">Email</label>
                    <input type="email" id="email" name="email" value="{{ old('email') }}" class="form-control @error('email') is-invalid @enderror" placeholder="you@example.com" required autofocus>
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="form-check">
                        ...
                        <label class="form-check-label small" for="remember_me" style="color: var(--js-text-secondary);">Remember me</label>
                    </div>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="small text-decoration-none" style="color: var(--js-accent-hover);">Forgot password?</a>
                    @endif
                </div>
            ...
            <p class="text-center mt-3 mb-0 small" style="color: var(--js-text-muted);">
                Don't have an account? <a href="{{ route('register') }}" class="text-decoration-none fw-medium" style="color: var(--js-accent-hover);">Sign up</a>
            </p>
        </div>
    </div>
